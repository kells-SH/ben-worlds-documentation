...
function loadDocByPath(path) {
  ...
  // Check if we have Jekyll URLs available (from docs-jekyll.html)
  if (window.docsUrls && window.docsUrls[path]) {
    // Use Jekyll's real URL - this fixes GA tracking!
    window.location.href = window.docsUrls[path];
    return;
  }
